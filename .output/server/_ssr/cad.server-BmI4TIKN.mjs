...
function videoPrompt(b) {
	return [
		...
		b.brief,
		b.dimensions ? `Approximate dimensions ${b.dimensions} ${b.units ?? ""}.` : "",
...
async function generateImage(prompt, referenceUrls = []) {
	...
	if (!url) throw new Error("The image generator returned no image. Try rephrasing the brief.");
	return url;
}
...
async function readVideoJob(id) {
	const res = await fetch(`${GATEWAY}/videos/${id}`, { headers: { Authorization: `Bearer ${key()}` } });
	if (!res.ok) throw new Error(await friendlyError(res));
	return await res.json();
}
async function downloadVideo(id) {
	const res = await fetch(`${GATEWAY}/videos/${id}/content`, { headers: { Authorization: `Bearer ${key()}` } });
	if (!res.ok) throw new Error(await friendlyError(res));
	return await res.arrayBuffer();
}
async function friendlyError(res) {
	const text = await res.text().catch(() => "");
	if (res.status === 429) return "The AI preview service is busy right now. Try again in a minute.";
	if (res.status === 402) return "AI preview credits have run out. Top up to keep generating.";
...
//#endregion
export { IMAGE_VARIANTS, createVideoJob, dataUrlToBytes, downloadVideo, generateImage, imagePrompt, readVideoJob, videoPrompt };
