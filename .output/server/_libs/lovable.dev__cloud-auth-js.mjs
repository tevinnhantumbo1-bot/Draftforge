...
var DEFAULT_DESKTOP_LOCALHOST_REDIRECT_URI =
  "http://127.0.0.1/iframe-oauth/callback";
...
function processOAuthResponse(data, expectedState) {
  if (data.state !== expectedState)
    return { error: /* @__PURE__ */ new Error("State is invalid") };
  if (data.error) {
...
function createAuth(config = {}) {
  ...
  async function signInWithOAuth(provider, opts = {}) {
    ...
    if (!popup && (isMobileApp || isDesktopApp)) {
      ...
      const webViewTimeoutPromise = new Promise((_, reject) => {
        webViewTimeoutId = setTimeout(() => {
          reject(
            /* @__PURE__ */ new Error("OAuth timed out waiting for response"),
          );
    ...
    try {
      return processOAuthResponse(
        await Promise.race([messagePromise, popupClosedPromise]),
        state,
      );
    } catch (error) {
      return {
