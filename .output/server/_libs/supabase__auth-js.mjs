...
function validatePKCEFlowId(flowId) {
  return typeof flowId === "string" && PKCE_FLOW_ID_PATTERN.test(flowId)
    ? flowId
    : null;
}
...
var pkceVerifierSlotKey = (storageKey, flowId) =>
...
async function storePKCEVerifier(
  ...
  await setItemAsync(
    storage,
    pkceVerifierSlotKey(storageKey, flowId),
    verifier,
  );
  const index = (await getPKCEFlowIndex(storage, storageKey)).filter(
    (id) => id !== flowId,
  );
  index.push(flowId);
...
async function retrievePKCEVerifier(storage, storageKey, flowId) {
  if (flowId) {
    const verifier = await getItemAsync(
      storage,
      pkceVerifierSlotKey(storageKey, flowId),
    );
    return {
...
async function removePKCEVerifier(storage, storageKey, flowId) {
  ...
  if (
    slotValue != null &&
    slotValue === (await getItemAsync(storage, legacyKey))
  )
    await removeItemAsync(storage, legacyKey);
}
/**
...
var GoTrueClient = class GoTrueClient {
  ...
  async initialize() {
    ...
    const queue =
      (_a = this._pendingInitNotifications) !== null && _a !== void 0 ? _a : [];
    this._pendingInitNotifications = null;
    ...
    try {
      ...
      if ("email" in credentials) {
        ...
        if (this.flowType === "pkce")
          [codeChallenge, codeChallengeMethod, flowId] =
            await this._getCodeChallengeAndMethod();
  ...
  async _exchangeCodeForSession(authCode, options) {
    ...
    const requestedFlowId = hasExplicitFlowId
      ...
      : isBrowser()
        ? validatePKCEFlowId(
            parseParametersFromURL(window.location.href)[PKCE_FLOW_ID_PARAM],
          )
        : null;
  ...
  async signInWithSSO(params) {
    ...
    try {
      ...
      if (
        ...
        isBrowser() &&
        !((_d = params.options) === null || _d === void 0
          ? void 0
          : _d.skipBrowserRedirect)
      )
  ...
  async _isPKCECallback(params) {
    ...
    if (
      ...
      (await getItemAsync(
        this.storage,
        pkceVerifierSlotKey(this.storageKey, flowId),
      ))
    )
      return true;
  ...
  async resetPasswordForEmail(email, options = {}) {
    ...
    if (this.flowType === "pkce")
      [codeChallenge, codeChallengeMethod, flowId] =
        await this._getCodeChallengeAndMethod(true);
    try {
  ...
  async _handleProviderSignIn(provider, options) {
    const { url, flowId } = await this._getUrlForProvider(
      `${this.url}/authorize`,
      provider,
      {
        redirectTo: options.redirectTo,
  ...
  _maybeAppendFlowIdToRedirect(redirectTo, flowId) {
    if (
      !redirectTo ||
      !flowId ||
      !this.experimental.appendPkceFlowIdToRedirects
    )
